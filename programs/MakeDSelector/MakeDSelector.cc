...
void Print_HeaderFile(
    string locSelectorBaseName, DTreeInterface *locTreeInterface,
    map<int, map<int, pair<Particle_t, string>>> &locComboInfoMap);
void Print_SourceFile(
    string locSelectorBaseName, DTreeInterface *locTreeInterface,
    map<int, map<int, pair<Particle_t, string>>> &locComboInfoMap,
    bool extraDefaults);
void Print_HeaderFile_MCGen(string locSelectorBaseName,
                            DTreeInterface *locTreeInterface);
void Print_SourceFile_MCGen(string locSelectorBaseName,
                            DTreeInterface *locTreeInterface);

inline bool IsRelevant(const string &name) {
  return (name != "ComboBeam" && name != "Target" &&
          name.find("Decaying") == string::npos);
}
...
int main(int argc, char *argv[]) {
  ...
  DTreeInterface *locTreeInterface =
      new DTreeInterface(locTree, true); // true: is input
...
void Print_Usage(void) {
  cout << endl
       ...
       << "  selector_base_name  : base name for generated DSelector files"
       << endl
       << endl;
}
...
void Print_HeaderFile(
  ...
  locHeaderStream << endl;
  locHeaderStream << "class " << locSelectorName << " : public DSelector"
                  << endl;
  locHeaderStream << "{" << endl;
  ...
  locHeaderStream << "		bool dIsPARAFlag; //else is PERP or AMO"
                  << endl;
  locHeaderStream << endl;
  ...
  locHeaderStream
      ...
      << endl;
  locHeaderStream << endl;
  locHeaderStream << "		//CREATE REACTION-SPECIFIC PARTICLE ARRAYS"
                  << endl;
  locHeaderStream << endl;
  ...
  for (; locStepIterator != locComboInfoMap.end(); ++locStepIterator) {
    ...
    locHeaderStream << "		DParticleComboStep* dStep"
                    << locStepIndex << "Wrapper;" << endl;

    map<int, pair<Particle_t, string>> &locStepInfoMap =
        locStepIterator->second;
    map<int, pair<Particle_t, string>>::iterator locParticleIterator =
        locStepInfoMap.begin();
    for (; locParticleIterator != locStepInfoMap.end(); ++locParticleIterator) {
      ...
      if (locPID == UnknownParticle) {
        ...
        locHeaderStream << "		DBeamParticle* dComboBeamWrapper;"
                        << endl;
        ...
        {
          locHeaderStream << "		DKinematicData* d" << locParticleName
                          << "Wrapper;" << endl;
        }
      } else if (locParticleName.substr(0, 7) == "Missing") {
        string locBranchName = locParticleName + string("__P4_KinFit");
        if (locTreeInterface->Get_Branch(locBranchName) !=
            NULL) // else not reconstructed
        {
          locHeaderStream << "		DKinematicData* d" << locParticleName
                          << "Wrapper;" << endl;
        }
      } else if (ParticleCharge(locPID) != 0) {
        locHeaderStream << "		DChargedTrackHypothesis* d"
                        << locParticleName << "Wrapper;" << endl;
      } else {
        locHeaderStream << "		DNeutralParticleHypothesis* d"
                        << locParticleName << "Wrapper;" << endl;
      }
    }
    locHeaderStream << endl;
  ...
  for (locStepIterator = locComboInfoMap.begin();
       locStepIterator != locComboInfoMap.end(); ++locStepIterator) {
    int locStepIndex = locStepIterator->first;
    if (locStepIndex != 0) {
      locHeaderStream << endl;
    }
    ...
    locHeaderStream << "	dStep" << locStepIndex
                    << "Wrapper = dComboWrapper->Get_ParticleComboStep("
                    << locStepIndex << ");" << endl;
    ...
    for (; locParticleIterator != locStepInfoMap.end(); ++locParticleIterator) {
      ...
      if (locPID == UnknownParticle) {
        ...
        locHeaderStream
            << "	dComboBeamWrapper = static_cast<DBeamParticle*>(dStep"
            ...
            (locParticleIndex < 0)) // else not reconstructed
        {
          locHeaderStream << "	d" << locParticleName << "Wrapper = dStep"
                          << locStepIndex << "Wrapper->Get_InitialParticle();"
                          << endl;
        }
      } else if (locParticleName.substr(0, 7) == "Missing") {
        string locBranchName = locParticleName + string("__P4_KinFit");
...
void Print_SourceFile(
  ...
  locSourceStream << "void " << locSelectorName << "::Init(TTree *locTree)"
                  << endl;
  locSourceStream << "{" << endl;
  ...
  locSourceStream
      ...
      << endl;
  locSourceStream << "	// // For histogramming the phi mass in phi -> K+ K-"
                  << endl;
  locSourceStream << "	// // Be sure to change this and dAnalyzeCutActions to "
                     "match reaction"
                  << endl;
  locSourceStream << "	std::deque<Particle_t> MyPhi;" << endl;
  locSourceStream << "	MyPhi.push_back(KPlus); MyPhi.push_back(KMinus);"
                  << endl;
  locSourceStream << endl;
  ...
  if (extraDefaults) {
    ...
    locSourceStream << "}" << endl;
    locSourceStream << endl;
    locSourceStream << "Bool_t " << locSelectorName
                    << "::Process(Long64_t locEntry)" << endl;
    locSourceStream << "{" << endl;
    ...
    locSourceStream << "	// simple or elaborate selection criteria, run "
                    ...
                    << endl;
    locSourceStream << "	// of the event and typically fill histograms."
                    << endl;
    locSourceStream << "	//" << endl;
    locSourceStream
    ...
    locSourceStream
        ...
        << endl;
    locSourceStream << "	// The return value is currently not used."
                    << endl;
    locSourceStream << endl;
    ...
    locSourceStream
        ...
        << endl;
    locSourceStream << "		dPreviousRunNumber = locRunNumber;"
                    << endl;
    locSourceStream << "	}" << endl;
    locSourceStream << endl;
    locSourceStream << "	/********************************************* "
    ...
    locSourceStream << "	dAnalyzeCutActions->Reset_NewEvent(); // "
                    ...
                    << endl;
    locSourceStream << endl;
    locSourceStream << "	//PREVENT-DOUBLE COUNTING WHEN HISTOGRAMMING"
                    << endl;
    ...
    locSourceStream
        ...
        << endl;
    locSourceStream << endl;
    locSourceStream << "	//INSERT USER ANALYSIS UNIQUENESS TRACKING HERE"
                    << endl;
    locSourceStream << endl;
    locSourceStream << "	/**************************************** "
    ...
    locSourceStream
        ...
        << endl;
    locSourceStream << endl;
    locSourceStream << "	TLorentzVector locMyP4(4.0, 3.0, 2.0, 1.0);"
                    << endl;
    locSourceStream << "	dTreeInterface->Fill_TObject<TLorentzVector>("
                       "\"my_p4\", locMyP4);"
                    << endl;
    locSourceStream << endl;
    locSourceStream << "	for(int loc_i = 0; loc_i < locMyInt; ++loc_i)"
                    << endl;
    locSourceStream
        << "		"
    ...
    locSourceStream
        ...
        << endl;
    locSourceStream << "	{" << endl;
    locSourceStream << "		dComboWrapper->Set_ComboIndex(loc_i);"
                    << endl;
    ...
    locSourceStream << "	for(const auto& loc_combo : loc_combos)"
                    << endl;
    locSourceStream << "	{" << endl;
    locSourceStream << "		UInt_t loc_i = loc_combo.first;"
                    << endl;
    ...
    for (; locStepIterator != locComboInfoMap.end(); ++locStepIterator) {
      ...
      map<int, pair<Particle_t, string>>::iterator locParticleIterator =
          locStepInfoMap.begin();
      for (; locParticleIterator != locStepInfoMap.end();
           ++locParticleIterator) {
        Particle_t locPID = locParticleIterator->second.first;
        string locParticleName = locParticleIterator->second.second;
        ...
        if (locPID == UnknownParticle) {
          ...
          locSourceStream << "		Int_t loc" << locParticleName
                          << "TrackID = d" << locParticleName
                          << "Wrapper->Get_TrackID();" << endl;
        } else {

          locSourceStream << "		Int_t loc" << locParticleName
                          << "NeutralID = d" << locParticleName
      ...
      for (; locParticleIterator != locStepInfoMap.end();
        ...
        if (locPID == UnknownParticle) {
          ...
          if ((locTreeInterface->Get_Branch(locBranchName) != NULL) &&
              (locParticleIndex < 0)) // else not reconstructed
          {
            locSourceStream << "		TLorentzVector loc"
                            << locParticleName << "P4 = d" << locParticleName
                            << "Wrapper->Get_P4();" << endl;
          }
        } else if (locParticleName.substr(0, 7) == "Missing") {
          string locBranchName = locParticleName + string("__P4_KinFit");
        ...
        } else // detected
        {
          locSourceStream << "		TLorentzVector loc" << locParticleName
                          << "P4 = d" << locParticleName << "Wrapper->Get_P4();"
                          << endl;
        }
      }
    }
    ...
    locSourceStream << "        // Need to uncomment the section computing "
                    ...
                    << endl;
    locSourceStream << "		//if(locUsedSoFar_Event == false)"
                    << endl;
    locSourceStream << "		//{" << endl;
    locSourceStream << "			// Fill the histogram only "
                       "when the beam bunch is in-time. "
                    << endl;
    locSourceStream << "			//if(!locRelBeamBucket)"
                    << endl;
    locSourceStream << "			//{" << endl;
    locSourceStream << "			//	"
    ...
    for (locStepIterator = locComboInfoMap.begin();
      ...
      for (; locParticleIterator != locStepInfoMap.end();
        ...
        if (locPID == UnknownParticle) {
          ...
          locSourceStream << "		locUsedThisCombo_MissingMass["
                          << EnumString(locPID) << "].insert(loc"
                          << locParticleName << "TrackID);" << endl;
        } else {
          locSourceStream << "		locUsedThisCombo_MissingMass["
                          << EnumString(locPID) << "].insert(loc"
                          << locParticleName << "NeutralID);" << endl;
        }
      }
    }
    locSourceStream << endl;

    locSourceStream << "		//compare to what\'s been used so far"
                    << endl;
    locSourceStream
        << "		"
    ...
    locSourceStream
        ...
        << endl;
    locSourceStream << endl;
    locSourceStream << "		// RECOMMENDED: FILL ACCIDENTAL WEIGHT"
                    << endl;
    locSourceStream
        << "		// "
    ...
    if (extraDefaults) {
      ...
      static const vector<string> order = {
          "Pi0",    "PiPlus", "PiMinus", "Photon",  "KLong",
          "KShort", "KPlus",  "KMinus",  "Neutron", "Proton"};
      auto typeIndex = [&](const string &s) {
        for (size_t i = 0; i < order.size(); ++i)
      ...
      for (auto &step : locComboInfoMap) {
        for (auto &p : step.second) {
          ...
          locSourceStream << "    // lab angles for " << name << "\n";
          locSourceStream
              << "    "
          ...
          locSourceStream
              << "    "
                 "dFlatTreeInterface->Fill_Fundamental<Double_t>(\"costh_lab_"
              << name << "_measured\", loc" << name
              << "P4_Measured.Vect().CosTheta());\n";
          locSourceStream
              << "    dFlatTreeInterface->Fill_Fundamental<Double_t>(\"phi_lab_"
              << name << "\",   loc" << name << "P4.Vect().Phi());\n";
          locSourceStream
              << "    dFlatTreeInterface->Fill_Fundamental<Double_t>(\"phi_lab_"
              << name << "_measured\",   loc" << name
              << "P4_Measured.Vect().Phi());\n";
        }
      }
      // GJ & Helicity
      // for (auto &step : locComboInfoMap) {
  ...
  locSourceStream << "		Fill_FlatTree(); //for the active combo"
                  << endl;
  ...
  locSourceStream << "		double locEnergy = dThrownBeam->Get_P4().E();"
                  << endl;
  locSourceStream << endl;
  locSourceStream << "	//Loop over throwns" << endl;
  locSourceStream << "	for(UInt_t loc_i = 0; loc_i < Get_NumThrown(); ++loc_i)"
                  << endl;
  locSourceStream << "	{" << endl;
  ...
  locSourceStream
      << "	//Loop over beam particles (note, only those appearing in "
         "combos are present)"
      << endl;
  locSourceStream << "	for(UInt_t loc_i = 0; loc_i < Get_NumBeam(); ++loc_i)"
                  << endl;
  locSourceStream << "	{" << endl;
  locSourceStream << "		//Set branch array indices corresponding to "
                     "this particle"
  ...
  locSourceStream << "		//Set branch array indices corresponding to "
                     "this particle"
                  << endl;
  locSourceStream << "		dChargedHypoWrapper->Set_ArrayIndex(loc_i);"
                  << endl;
  locSourceStream << endl;
  locSourceStream << "		//Do stuff with the wrapper here ..." << endl;
  locSourceStream << "	}" << endl;
  ...
  locSourceStream << "		//Set branch array indices corresponding to "
                     "this particle"
                  << endl;
  locSourceStream << "		dNeutralHypoWrapper->Set_ArrayIndex(loc_i);"
                  << endl;
  locSourceStream << endl;
  locSourceStream << "		//Do stuff with the wrapper here ..." << endl;
  locSourceStream << "	}" << endl;
  ...
  locSourceStream << "	}" << endl;
  locSourceStream << "	if(!locIsEventCut && dOutputTreeFileName != \"\")"
                  << endl;
...
void Print_SourceFile_MCGen(string locSelectorBaseName,
  ...
  locSourceStream << "	//" << endl;
  locSourceStream << "	// The processing can be stopped by calling Abort()."
                  << endl;
  locSourceStream
      << "	// Use fStatus to set the return value of TTree::Process()."
      << endl;
  ...
  locSourceStream << endl;
  locSourceStream << "		//Do stuff with the wrapper here ..." << endl;
  locSourceStream << "		Particle_t locPID = dThrownWrapper->Get_PID();"
                  << endl;
  locSourceStream
